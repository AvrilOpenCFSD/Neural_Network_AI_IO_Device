...
// public.
        ...
        {
            _List_Of_Binary_Paths = new OpenAvrilNNAI_OpenAvrilConcurrency_IO_Praise_Nth.Binary[numberOfBinaryValues];
            while (Get_List_Of_Binary_Paths() == null) { }
            for (byte index =0; index < numberOfBinaryValues; index++)
            {
                Set_Item_On_List_Of_Binary_Paths(index, new OpenAvrilNNAI_OpenAvrilConcurrency_IO_Praise_Nth.Binary());
            }
        }
        public void Create_List_Of_Constant_Paths(byte numberOfResetToConstantValues)
        {
            _List_Of_Constant_Paths = new OpenAvrilNNAI_OpenAvrilConcurrency_IO_Praise_Nth.Constant[numberOfResetToConstantValues];
            while (Get_List_Of_Constant_Paths() == null) { }
            for(byte index = 0; index < numberOfResetToConstantValues; index++)
            {
                Set_Item_On_List_Of_Constant_Paths(index, new OpenAvrilNNAI_OpenAvrilConcurrency_IO_Praise_Nth.Constant());
            }
        }
        public void Create_List_Of_Linear_Paths(byte numberOfInputValuesForNode)
        {
            _List_Of_Linear_Paths = new OpenAvrilNNAI_OpenAvrilConcurrency_IO_Praise_Nth.Linear[numberOfInputValuesForNode];
            while (Get_List_Of_Linear_Paths() == null) { }
            for (byte index = 0; index < numberOfInputValuesForNode; index++)
            {
                Set_Item_On_List_Of_Linear_Paths(index, new OpenAvrilNNAI_OpenAvrilConcurrency_IO_Praise_Nth.Linear());
            }
        }
